    ...
    <div className="cursor-pointer">
      <Accordion type="single" collapsible className="border-b-2 mr-4">
        <AccordionItem value={`item-${segments[0].sessionId}`}>
            ...
            <div className="flex w-full justify-between font-bold cursor-pointer">
              ...
              <div className="text-xl flex items-center gap-x-2">
                ...
                {segments
                  .reduce(
                    (acc, val) =>
                      (acc += val.learningSessionSegmentsTable.timespent / 60),
                    0,
                  )
                  .toFixed(2)}
                <Tooltip>
          ...
          <AccordionContent className="flex flex-col gap-4 mb-3 text-balance mx-10">
            <Table>
              <TableBody>
                {segments.map((segment, index) => (
                  <TableRow key={index}>
                    <TableCell className="font-bold">
                      {segment.learningSessionSegmentsTable.notes}
                    </TableCell>
                    <TableHead className="text-end font-bold">
                      Session {index + 1}:{" "}
                      {(
                        segment.learningSessionSegmentsTable.timespent / 60
                      ).toFixed(2)}{" "}
                      hr(s)
                    </TableHead>
                  </TableRow>
